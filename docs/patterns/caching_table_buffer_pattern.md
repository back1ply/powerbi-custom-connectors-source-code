...
Because Power Query's M engine evaluates code _lazily_, it will often re-evaluate a variable if it is referenced multiple times in a script.
...
If you don't explicitly cache that list of projects, Power Query might re-fire the `GET /projects` API call for _every single project in the list_ as it builds the UI!
