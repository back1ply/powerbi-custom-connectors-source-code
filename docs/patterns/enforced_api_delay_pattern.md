...
While the [API Retries Pattern](api_retries_pattern.md) handles _reacting_ to 429 errors after they happen, sometimes you need to _proactively_ throttle Power Query to prevent the errors from occurring in the first place.
...
_Warning: Be careful using heavy throttling on large datasets. If an API requires a 5-second delay between pages, and the user tries to download 1,000 pages of data, the refresh will take over 1 hour and may time out in the Power BI Service._
