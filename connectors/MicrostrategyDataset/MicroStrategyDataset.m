...
// Global logging flag - currently hardcoded to false, but can be enabled for debugging purposes.
IS_LOGGING_ENABLED = false;
// Default logging level to be passed to Strategy Library REST API.
LOGGING_LEVEL= "ERROR";
...
MicroStrategyDataset.Contents.Impl = (libraryUrl as text, optional authMode as text, optional options as record) as table =>
	let
		...
		auth = if _authMode = Extension.LoadString("StandardAuth") or _authMode  = Extension.LoadString("LDAPAuth") then null else Authenticate(restApiUrl, _authMode, timeout, credentials),

		connection = [
			auth = auth,
			authMode = _authMode,
			isRefreshEnabled = _authMode = "OIDC",
	...
	in
		FindProjects(connection);

// Function to authenticate user and return the token and cookie
Authenticate = (restApiUrl as text, authMode as text, timeout as number, credentials as record, optional loginLogout as logical, optional defaultAuth as record) =>
    if authMode = "OIDC" then
		null
    else if authMode = "Library" then
        // For initial library login - return standard Rest.Delegate, for login logout workflow just return current, default auth
        if loginLogout = false or loginLogout = null then
            Rest.Delegate(restApiUrl, credentials[access_token], timeout)
        else
            defaultAuth
    else if loginLogout = true then
        // Login / Logout after requests workflow
        Rest.Login(restApiUrl, authMode, credentials, timeout, true)
    else
        // Initial login attempt
        Rest.Login(restApiUrl, authMode, credentials, timeout, false);
...
HandleNavigation = (connection as record, projectId as text, obj as record) =>
	let
		...
		objId = obj[id],
		kind = if objType = ObjectTypes[Folder] then "Folder" else if objSubType = ObjectSubTypes[ReportCube] or objSubType = ObjectSubTypes[ReportEmmaCube] then "CubeView" else "Sheet",
...
HandleDataset = (initialConnection as record, projectId as text, datasetId as text, isCube as logical, datasetName as text) =>
	let
		promptConnection = CreateUpdatedConnection(initialConnection),
		isV2 = Status.IsV2Compatible(promptConnection[status]),
		...
		prompts = Rest.GetPrompts(promptConnection, projectId, datasetId),
		...
		// We log in just to get initial instance data and pass it, which we immediatly log out after
		dataConnection = CreateUpdatedConnection(initialConnection),
		instanceData = if not hasUnsupportedPrompts then InitInstance(dataConnection, projectId, datasetId, isCube, isV2) else error Extension.LoadString("ErrorPrompted"),
		...
		if not hasPrompts then
			let
				view = MicroStrategyDataset.View(initialConnection, Instance, projectId, datasetId, isCube, isV2),
				logoutInstance = GetResultAndLogout(dataConnection, dataConnection[auth], view)
			...
			in
				if isCube then
					...
					NavigationTable.FromRows({
						{datasetName, "Import", logoutInstance, "Table", "Table", true}
					})
		else
			if Status.IsPromptCompatible(dataConnection[status]) then
				CreatePromptNavTable(dataConnection, prompts, projectId, datasetId, instanceId, isCube, isV2, datasetName, Instance)
			else
				error Extension.LoadString("ErrorPromptedOldEnv"),
				

		resultedData = GetResultAndLogout(promptConnection, promptConnection[auth], result)
		in
			resultedData;
	...
	let
		requestBody = if isCube or not isV2 then
				...
				let
					...
					allMetricsAndAttributes = GetAllMetricsAndAttributes(grid),
					attributes = allMetricsAndAttributes[attributes],
					metrics = allMetricsAndAttributes[metrics],
					hasMetrics = not List.IsEmpty(metrics),
					bodyWithAttributes = [
						attributes = List.Transform(attributes, each [id = _[id]])
					],
					objects =
					if hasMetrics then
						Record.AddField(
							bodyWithAttributes,
							"metrics",
							List.Transform(metrics, each [id = _[id]])
						)
					else
						bodyWithAttributes
				in
		...
		hasPrompts = not isCube and not List.IsEmpty(prompts),
		instance = Rest.PostInstance(limitedConnection, projectId, datasetId, isCube, isV2, requestBody, hasPrompts),
		instanceId= instance[instanceId],
		readyInstance =  Rest.WaitForInstance(connection,projectId,datasetId,instanceId, isCube),
		instanceData= Rest.GetInstance(limitedConnection, projectId, datasetId, instanceId, isCube, true, readyInstance[status])
	in
		instanceData;
...
MicroStrategyDataset.View = (initialConnection, Instance as record, projectId as text, datasetId as text, isCube as logical, isV2 as logical, optional promptInstance as record) =>
	let
		getColumnHeaders = if isV2 then MicroStrategyDataset.ColumnHeadersFromV2 else MicroStrategyDataset.ColumnHeadersFromV1,
		toTableRows = if isV2 then MicroStrategyDataset.FromV2 else MicroStrategyDataset.FromV1,
		connectionLimitInitial = initialConnection[limit],
		...
		View = (state as record) => Table.View(null, [
			 ...
			 */
			GetRows = () => let
				getRowsResult = GetRows(initialConnection, projectId, datasetId, isCube, isV2, state, toTableRows, promptInstance),
	...
	in
		View([offset = 0, limit = connectionLimitInitial, rowCount = Instance[rowCount]]);

CreatePromptNavTable = (connection as record, prompts as list, projectId as text, reportId as text, instanceId as text, isCube as logical, isV2 as logical, reportName as text, instance as record) =>
	let
		...
		defaultOptions = {
			...
			{Extension.LoadString("PromptsCloseAll") & " - " & reportName, "PromptsClosed", CloseAllPrompts(CreateAnswerPrompts(instanceMeta, isV2, instance)), "View", "View", true}
		},
		rows = defaultOptions
	in
		let
...
CreateAnswerPrompts = (instanceMeta as record, isV2 as logical, instance as record) =>
	let
		...
		AnswerPrompts.Impl = (optional answers as list, optional messageName as text) =>
			if answers = null then
				...
				let
					isV2 = Status.IsV2Compatible(instanceMeta[status]),
					getColumnHeaders = if isV2 then MicroStrategyDataset.ColumnHeadersFromV2 else MicroStrategyDataset.ColumnHeadersFromV1,
				...
				in
					if List.IsEmpty(unansweredPrompts) then
						let
							instanceStatus =  Rest.WaitForInstance(instanceMeta,projectId,reportId,instanceId, false),
							newInstanceData = ((_) => GetSchema(instanceMeta, projectId, reportId, instanceId, false, isV2))(instanceStatus),
							NewInstance =

							[
								Data = newInstanceData,
								ID = newInstanceData[instanceId],
								rowCount = (if isV2 then Data else Data[result])[data][paging][total],
								headers = getColumnHeaders(Data),
								deDuplicatedHeaders = ColumnHeaders.DeDuplicate(headers),
								tableType = ColumnHeaders.ToTableType(deDuplicatedHeaders)
						...
						in
							MicroStrategyDataset.View(instanceMeta, NewInstance, projectId, reportId, false, isV2 , NewInstance)
					else
						CreateAnswerPrompts(instanceMeta & [ prompts = unansweredPrompts ], isV2, instance)
	in
 ...
 */
GetRows = (initialConnection as record, projectId as text, datasetId as text, isCube as logical, isV2 as logical, paging as record, toTableRows as function, optional promptInstance as record) =>
	let
		// Create connection used for retrieing data 
		dataConnection = if promptInstance = null then CreateUpdatedConnection(initialConnection) else initialConnection,
		connectionLimitInitial = dataConnection[limit],
		instanceData =  if promptInstance = null then  InitInstance(dataConnection, projectId, datasetId, isCube, isV2) else promptInstance,
		usedId = if promptInstance = null then instanceData[instanceId] else instanceData[ID],
		...
		fetchPage = (pair) =>
			let
				limitedConnection = dataConnection & [limit = pair{1}]
			in
				Rest.GetInstance(limitedConnection, projectId, datasetId, usedId, isCube, isV2, pair{0}),
		fetchedPages = List.Transform(pagesToFetch, fetchPage),
		resultCombined = List.Buffer(List.Combine(List.Transform(fetchedPages, toTableRows))),
		resultWithLogout = GetResultAndLogout(dataConnection, dataConnection[auth], resultCombined, true, true)
	in
		// Create a list of the first page and all other fetched pages, combine the resulting list of pages into one gigantic page. (still in list of lists form)
		 if promptInstance = null then resultWithLogout else resultCombined;
...
MicroStrategyDataset.FromV2 = (jResult as record) =>
	let
		...
		lookupAttributeValue = (x) => if Record.FieldOrDefault(lookupAttribute(x), "formValues", null) <> null then lookupAttribute(x)[formValues] else {},
		...
		selectedRows = List.Select(rowPairs, (x) => not isSubtotalRow(x))
	in
...
MicroStrategyDataset.ColumnHeadersFromV2 = (jResult as record) =>
	let
		...
		metricHeaders = List.Transform(metrics, V2.getMetricHeader)
	in
		if hasMetrics then
			List.Combine({attributeHeaders, metricHeaders})
		else
...
GetAllMetricsAndAttributes = (grid as record) =>
    let
        attributesInRows = List.Select(grid[rows], each (_[type] = "attribute" or _[type] = "consolidation")),
        attributesInColumns = List.Select(grid[columns], each (_[type] = "attribute" or _[type] = "consolidation")),
        metricsInRows = List.Select(grid[rows], each _[type] = "templateMetrics"),
        metricsInRowsElements = if List.IsEmpty(metricsInRows) then {} else metricsInRows{0}[elements],
        metricsInColumns = List.Select(grid[columns], each _[type] = "templateMetrics"),
        metricsInColumnsElements = if List.IsEmpty(metricsInColumns) then {} else metricsInColumns{0}[elements],
        allMetrics = List.Combine({metricsInColumnsElements, metricsInRowsElements}),
        allAttributes = List.Combine({attributesInRows, attributesInColumns})
    in
        [metrics = allMetrics, attributes = allAttributes];
...
CredsFromHeaders = (headers as record) as record =>
	let
		cookiesBlackList = {"Expires", "Max-Age", "Domain", "Path", "Secure", "HttpOnly", "SameSite"},
		xtokenField = if Record.HasFields(headers, "X-MSTR-AuthToken") then "X-MSTR-AuthToken" else "x-mstr-authtoken",
		xcookieStringWithInvalidCharactersRemoved = Text.Replace(headers[#"Set-Cookie"], ", ", ""),
		xcookieList = List.Combine(List.Transform(Text.Split(xcookieStringWithInvalidCharactersRemoved, ","), (str) => Text.Split(str, ";"))),
		xcookieListWithBlackListApplied = List.Select(xcookieList, each RemoveIfOnBlackList(_, cookiesBlackList) ) 
	in
		[
			xtoken = Record.Field(headers, xtokenField),
			xcookie = Text.Combine(xcookieListWithBlackListApplied, "; ")
		];

RemoveIfOnBlackList = (cookieItem as text, blackList as list) as logical =>
	not List.MatchesAny(blackList, each Text.Contains(cookieItem, _, Comparer.OrdinalIgnoreCase));
...
Status.IsVersionCompatible = (status as record, version as text) =>
	let
		...
		iVer = try MicroStrategyDataset.VersionFromText(status[iServerVersion]) otherwise {},
		webVer = try MicroStrategyDataset.VersionFromText(status[webVersion]) otherwise {}
	in
...
CheckResponse = (response, targetUrl as text, optional retry as logical, optional requestData as record, optional libraryUrl, optional log as number) =>
	let
...
HandleResponse = (response, targetUrl as text,  optional retry as logical, optional requestData as record, optional libraryUrl, optional log as number) =>
    let
        checkedResponse = CheckResponse(response, targetUrl, retry, requestData, libraryUrl),
        result = Json.Document(checkedResponse)
    in
        result;


// Temporal workaround - Same as CheckResponse, but adapted to the instance status call. Due to a Strategy REST API bug, if the status 202, we need to mock a response body with status 0, since currently it returns an empty body.
CheckResponseForStatusCall = (response, targetUrl as text, optional retry as logical, optional requestData as record, optional libraryUrl, optional log as number) =>
	let
		bufferedResponse = Binary.Buffer(response),
		responseMeta = Value.Metadata(response),
		status = responseMeta[Response.Status]
	in
		 ...
		 */
		if status = 202 then
			Text.ToBinary("{""status"": 0}")
		else if bufferedResponse <> null and status <> null and status < 300 and status >= 200 then
			bufferedResponse meta responseMeta
		else
			// Retry functionality is currently implemented only for OIDC authentication
			if retry and status = 401 then
...
// Temporal workaround - Same as HandleResponse, but adapted to the instance status call. Due to a Strategy REST API bug, if the status 202, we need to mock a response body with status 0, since currently it returns an empty body.
HandleResponseForStatusCall = (response, targetUrl as text,  optional retry as logical, optional requestData as record, optional libraryUrl, optional log as number) =>
    let
        checkedResponse = CheckResponseForStatusCall(response, targetUrl, retry, requestData, libraryUrl),
        result = Json.Document(checkedResponse)
    in
        result;

LoginAndGetAuth = (connection as record) as record =>
    let
        credentials = Extension.CurrentCredential(),

        auth = try
            if connection[authMode] = "OIDC" then
                getCurrentCredentials(connection)
            else
                Authenticate(connection[restApiUrl], connection[authMode], connection[timeout], credentials, true, connection[auth])
        otherwise
		// Force re-authentication for OIDC when token expires
            Extension.CurrentCredential(true)
    in
        auth;

CreateUpdatedConnection = (initialConnection as record) =>
	let
		newAuth = LoginAndGetAuth(initialConnection),
		updatedConnection = Record.TransformFields(initialConnection, {{"auth", each newAuth}}),
		connection = updatedConnection
	in
		connection;

GetResultAndLogout = (connection as record, auth as record, functionResult as any, optional isRetry as logical, optional loginLogout as logical) as any =>
	let
		authMode = connection[authMode],

		isRetry = if isRetry = null then true else isRetry,
		logoutResult = Rest.Logout(connection, connection[restApiUrl], connection[timeout], isRetry, auth),
		logoutResultTrick = if logoutResult = null then functionResult else functionResult,
 
		result = if functionResult = null then logoutResultTrick else logoutResultTrick,

		// we are making logout for standard - dont do logout for OIDC & Library
		logoutOnlyForStandard = 
			if authMode = "OIDC" then
				functionResult
			else if authMode = "Library" then
				functionResult
...
// POST /auth/login
Rest.Login = (restApiUrl as text, authMode as text, credentials as record, _timeout as number, optional IsRetry as logical) =>
	let
...
Rest.GetPrivilege = (connection as record, projectId as text) as record =>
	let
		auth = LoginAndGetAuth(connection),
		...
		handleResponse = HandleResponse(response, requestUrl, connection[isRefreshEnabled], requestData, connection[libraryUrl]),
		resultWithLogout = GetResultAndLogout(connection, auth, handleResponse)
	in
		resultWithLogout;
...
Rest.GetDefinition = (connection as record, projectId as text, datasetId as text, isCube as logical, isV2 as logical) =>
	let
		...
		logContent= Text.FromBinary(Json.FromValue(requestOptionsWithHeaders)),
		log=LogIfEnabled(connection, LOGGING_LEVEL, "Getting definition: " & logContent, "GetDefinition", "Getting Definition", requestUrl)
	in
		HandleResponse(response, requestUrl, connection[isRefreshEnabled], requestData, connection[libraryUrl],log);
...
Rest.GetPrompts = (connection as record, projectId as text, reportId as text) =>
	let
		...
		logContent= Text.FromBinary(Json.FromValue(requestOptionsWithHeaders)),
		log = LogIfEnabled(connection, LOGGING_LEVEL, "Getting prompts: " & logContent, "GetPrompts", "Getting prompts", requestUrl)
	in
		HandleResponse(response, requestUrl, connection[isRefreshEnabled], requestData, connection[libraryUrl],log);
...
Rest.PostInstance = (connection as record, projectId as text, datasetId as text, isCube as logical, isV2 as logical, requestBody as binary, hasPrompts as logical) =>
	let
		auth = getCurrentCredentials(connection),
		restApiUrl = connection[restApiUrl],
		...
		requestData = [
			...
			requestHeadersWithoutCreds = [
				...
				#"Content-Type"="application/json",
				#"X-MSTR-ProjectID"=projectId,
				Prefer ="respond-async" // Async response ensures that the instance is created and the response is returned immediately, even if the instance is not ready yet.
			],
			requestOptions = [
				Query =
					let
				...
				IsRetry = true,
				Content = (if hasPrompts then Json.FromValue([]) else requestBody),
				ManualCredentials = true,
				ManualStatusHandling = {400, 401, 500},
				Timeout=#duration(0, 0, 0, connection[timeout])
			]
	    ...
	    logContent= Text.FromBinary(Json.FromValue(requestOptionsWithHeaders)) & " Request body: " & Text.FromBinary(requestBody),
		log = LogIfEnabled(connection, LOGGING_LEVEL, "Creating report instance : " & logContent, "PostInstance", "Create report instance", requestUrl),
		instance= HandleResponse(response, requestUrl, connection[isRefreshEnabled], requestData, connection[libraryUrl],log)
	in
		instance;

// Adds a delay and checks the status of the instance until it is ready or times out.
// The timeout is set to 120 minutes (7200 iterations of 1 second each).
Rest.WaitForInstance = (connection as record, projectId as text, reportId as text, instanceId as text, isCube as logical) =>
	let
		resultList = List.Generate(
			() => [
				Response = Rest.GetInstanceStatus(connection, projectId, reportId, instanceId, isCube),
				Status = Response[status],
				Count = 0
			],
			each (([Status] <> 1 and [Status] <> 2) or [Count] = 0) and [Count] < 7200,  // allow first iteration always - required to not have empty list
			each [
				Response = Function.InvokeAfter(() => Rest.GetInstanceStatus(connection, projectId, reportId, instanceId, isCube), #duration(0, 0, 0, 1)),
				Status = Response[status],
				Count = [Count] + 1
			],
			each [Response]
		),
		finalResult = if List.IsEmpty(resultList) then error "Instance creation took too long." else List.Last(resultList)
	in
		finalResult;


Rest.GetInstanceStatus = (connection as record,projectId as text,reportId as text,instanceId as text, isCube as logical) =>
	let
		auth = getCurrentCredentials(connection),
		restApiUrl = connection[restApiUrl],
		requestUrl = restApiUrl &  "reports/" & reportId & "/instances/" & instanceId & "/status",
		requestData = [
			requestUrl = requestUrl,
			requestHeadersWithoutCreds = [
				Accept = "application/json",
	    ...
	    logContent= Text.FromBinary(Json.FromValue(requestOptionsWithHeaders)),
		log = LogIfEnabled(connection, LOGGING_LEVEL, "Getting report status: " & logContent, "GetInstanceStatus", "Getting report status", requestUrl)
	in
		HandleResponseForStatusCall(response, requestUrl, connection[isRefreshEnabled], requestData, connection[libraryUrl],log);
...
Rest.GetInstance = (connection as record, projectId as text, datasetId as text, instanceId as text, isCube as logical, isV2 as logical, offset as number) =>
	let
		...
		logContent= Text.FromBinary(Json.FromValue(requestOptionsWithHeaders)),
		log = LogIfEnabled(connection, LOGGING_LEVEL, "Getting instance data: " & logContent, "GetInstance", "Getting Instance Data", requestUrl)
	in
		HandleResponse(response, requestUrl, connection[isRefreshEnabled], requestData, connection[libraryUrl],log);
...
Rest.GetInstancePrompts = (connection as record, projectId as text, reportId as text, instanceId as text) =>
	let
	    ...
	    logContent= Text.FromBinary(Json.FromValue(requestOptionsWithHeaders)),
		log = LogIfEnabled(connection, LOGGING_LEVEL, "Getting instance prompts: " & logContent, "GetInstancePrompts", "Getting instance prompts", requestUrl)
	in
		HandleResponse(response, requestUrl, connection[isRefreshEnabled], requestData, connection[libraryUrl], log);
...
Rest.PostInstancePromptsAnswers = (connection as record, projectId as text, reportId as text, instanceId as text, requestBody as binary) =>
	let
		...
		requestOptionsWithHeaders = Record.AddField(requestData[requestOptions], "Headers", requestHeaders),
		response= Web.Contents(requestUrl,requestOptionsWithHeaders),
	    logContent= Text.FromBinary(Json.FromValue(requestOptionsWithHeaders)) & " Request body: " & Text.FromBinary(requestBody),
		log = LogIfEnabled(connection, LOGGING_LEVEL, "Answering instance prompts: " & logContent, "PostInstancePrompt Answers", "Answering instance prompts", requestUrl)
	in
		CheckResponse(response, requestUrl, connection[isRefreshEnabled], requestData, connection[libraryUrl], log);
...
Rest.Logout = (connection as record, restApiUrl as text, _timeout as number, IsRetry as logical, auth as record) =>
	let
		requestUrl = restApiUrl & "auth/logout",
		requestBody = "{}",
		response = Web.Contents(
			requestUrl,
			[
	...
	in
		CheckResponse(response, requestUrl);

// Logging function - it logs to the server if logging is enabled, otherwise does nothing.
// PowerBI does not have a built-in logging mechanism, so we use the Strategy REST API server for logging.
LogIfEnabled = (connection as record, logLevel as text, message as text, ticketId as text, stackTrace as text, tier as text) =>
    if IS_LOGGING_ENABLED then
        Rest.LogToServer(connection, logLevel, message, ticketId, stackTrace, tier)
    else
        null;

// Function to log messages to the Strategy REST API server. 
Rest.LogToServer = (connection as record, logLevel as text, message as text, optional ticketId as text, optional stackTrace as text, optional tier as text) =>
    let    
        logPayload = Json.FromValue([
            level = logLevel,
            tier = if tier <> null then tier else "Application",
            stackTrace = if stackTrace <> null then stackTrace else "",
            message = message,
            ticketId = if ticketId <> null then ticketId else ""
        ]),
       
        requestUrl = connection[restApiUrl] & "logs",
        ...
        result = try Json.Document(response) otherwise null
    in
		status;
