...
// OAuth
oauthClientId = Text.FromBinary(Extension.Contents("client_id"));
oauthRedirectUri = "https://oauth.powerbi.com/views/oauthredirect.html";
// offline_access allows us to use refresh tokens
oauthTokenScope = "foundry-sql-server:query offline_access";

clientCredentialsTokenScope = "foundry-sql-server:query";
clientCredentialsRefreshIntervalMinutes = 15;
...
Foundry.ValidateCredentials = (multipassApiPath as text) =>
    let
        token = if Foundry.IsOauth()
            then Foundry.GetOauth()
            else if Foundry.IsClientCredentials()
                then Foundry.ClientCredentials(multipassApiPath)
                else if Foundry.IsToken()
                    then Foundry.GetToken()
                    else error Extension.LoadString("UnsupportedCredentialsError"),
...
Foundry.ClientCredentials = (multipassApiPath as text) =>
    let
        credentials = Extension.CurrentCredential(),
        tokenRecord = Foundry.ClientCredentialsExchange(multipassApiPath, credentials[Username], credentials[Password])
    in
...
Foundry.OdbcFlatNavTable = (connectionString as record, token as text) =>
    let
        credentials = Foundry.DriverCredentials(token),
        datasourceNavTable = Odbc.DataSource(connectionString, [
		    CredentialConnectionString = credentials,
...
Foundry.DriverCredentials = (connectorToken as text) =>
    let
        credential = Extension.CurrentCredential(),
        driverCredentials = if Foundry.IsClientCredentials()
            // If using client credentials, we pass these directly to the driver so it can handle token refresh for long queries
            // TODO(mgripton): Should we be passing the OAuth refresh token to the driver as well? Although our
            // refresh tokens can only be used once, so cannot be shared.
            then [ authMethod = "ClientCredentials", oauthClientId = credential[Username], oauthClientSecret = credential[Password] ]
            // Driver only needs a token, not a username
            else [ UID = "", PWD = connectorToken ]
    in
        driverCredentials;
...
// https://learn.microsoft.com/en-us/power-query/handling-authentication#accessing-the-current-credentials
...
Foundry.GetOauth = () => Extension.CurrentCredential()[access_token];

// Note that Key authentication also sets the Password field, so we need to check for Username as well
Foundry.IsClientCredentials = () => Record.HasFields(Extension.CurrentCredential(), "Username") and Record.HasFields(Extension.CurrentCredential(), "Password");

Foundry.IsToken = () => Record.HasFields(Extension.CurrentCredential(), "Key");
Foundry.GetToken = () => Extension.CurrentCredential()[Key];
...
Foundry.StartLogin = (resourceUrl, state, display) =>
    let
        ...
        multipassApiPath = Foundry.MultipassUri(baseUri),
    ...
    in
        [
            ...
            WindowHeight = 720,
            WindowWidth = 1024,
            Context = [MultipassApiPath = multipassApiPath, CodeVerifier = codeVerifier]
        ];
...
Foundry.FinishLogin = (context, callbackUri, state) =>
    ...
    in
        Foundry.TokenMethod(context[MultipassApiPath], tokenQueryArgs, []);
...
Foundry.Refresh = (resourceUrl, refreshToken) =>
    ...
    in
        Foundry.TokenMethod(multipassApiPath, tokenQueryArgs, []);
...
Foundry.ClientCredentialsExchange = (multipassApiPath as text, clientCredsClientId as text, clientCredsClientSecret as text) =>
    let
        tokenQueryArgs = [
            client_id = clientCredsClientId,
            client_secret = clientCredsClientSecret,
            grant_type="client_credentials",
            scope = clientCredentialsTokenScope
        ],
        timeCacheKey = Foundry.TimeIntervalKey()
    in
        Foundry.TokenMethod(multipassApiPath, tokenQueryArgs, [TimeCacheKey = timeCacheKey]);

// Powerbi caches responses from Web.Contents.
// We need a way to trigger powerbi to refresh cached client cred tokens, ideally without refreshing on every request.
// We can't use the expires_in value because it's relative, and may come from a previous cached call.
// Instead, create a key based on the current time truncated to some interval. We set this key as a header
// which will change after each time interval passes, triggering powerbi to re-run the token exchange method.
// (N.B. Based on testing, repeated requests are made and some are cached and some are not, but we still need to
// retrigger the ones that are.)
Foundry.TimeIntervalKey = () =>
    let
        now = DateTimeZone.UtcNow(),
        nowStartOfHour = Time.StartOfHour(now),
        currentMinute = Time.Minute(now),
        minuteInterval = Number.IntegerDivide(currentMinute, clientCredentialsRefreshIntervalMinutes),
        // Create a key with the current hour and minute divided into intervals.
        timeKey = Text.Combine({DateTimeZone.ToText(nowStartOfHour), Number.ToText(minuteInterval)})
    in
        timeKey;

Foundry.TokenMethod = (multipassApiPath as text, queryArgs as record, additionalHeaders as record) =>
    let
        baseHeaders = [ #"Content-type" = "application/x-www-form-urlencoded", Accept = "application/json" ],
        response = Web.Contents(multipassApiPath & "/oauth2/token", [
...
Foundry = [
    Authentication = [
        ...
        UsernamePassword = [
            Label=Extension.LoadString("ClientCredentialsAuthenticationName"),
            UsernameLabel=Extension.LoadString("ClientCredentialsAuthenticationUsernameLabel"),
            PasswordLabel=Extension.LoadString("ClientCredentialsAuthenticationPasswordLabel")
         ]
    ],
...
Foundry.Icons = [
    Icon16 = { Extension.Contents("Foundry32.png"), Extension.Contents("Foundry40.png"), Extension.Contents("Foundry48.png"), Extension.Contents("Foundry64.png") },
