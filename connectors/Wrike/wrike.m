...
//
client_id =  Extension.LoadString("client_id");
client_secret = Extension.LoadString("client_secret");
...
base_oauth_uri = Extension.LoadString("base_oauth_uri");
...
GetGlobalData = () => 
     ...
     selectedCaseAllTasks = GetSelectedAPICall("alltasks"),
    ...
    // globalCustomFieldsExpanded = Table.SelectRows(customFieldsExpanded, each ([spaceId] = null)),
     ...
     objects = #table(
            ...
            { "All Tasks", Wrike.GetAllPagesByNextLink("alltasks", selectedCaseAllTasks{0}), "Table", "Table", true },
...
CreateMultiLvlFolder = (folderTitle as text, folderId as text,ids as list, isSpace as logical) as table => 
    let
       ...
       // customFieldsExpandedInSpace = Table.SelectRows(customFieldsExpanded, each ([spaceId] = null or [spaceId] = folderId)),
        ...
        expandCustom =if(Table.IsEmpty(tbl)) then tbl else Table.ExpandTableColumn(addCustom, "Custom", {"id", "accountId", "title", "createdDate", "completedDate", "updatedDate", "description", "sharedIds", "parentIds", "childIds", "superParentIds", "scope", "hasAttachments", "permalink", "workflowId", "metadata", "customFields","inheritedCustomColumnIds"}, {"Custom.id", "Custom.accountId", "Custom.title", "Custom.createdDate", "Custom.completedDate", "Custom.updatedDate", "Custom.description", "Custom.sharedIds", "Custom.parentIds", "Custom.childIds", "Custom.superParentIds", "Custom.scope", "Custom.hasAttachments", "Custom.permalink", "Custom.workflowId", "Custom.metadata", "Custom.customFields","Custom.inheritedCustomColumnIds"}),
        rmvColumn =if(Table.IsEmpty(tbl)) then tbl else Table.RemoveColumns(expandCustom,{"Custom.id", "Custom.accountId", "Custom.createdDate", "Custom.completedDate", "Custom.updatedDate", "Custom.description", "Custom.sharedIds", "Custom.parentIds", "Custom.superParentIds", "Custom.scope", "Custom.hasAttachments", "Custom.permalink", "Custom.workflowId", "Custom.metadata", "Custom.customFields","Custom.inheritedCustomColumnIds"}),
...
GetSelectedAPICall = (tableType as text,optional id as text) => 
    let 
        CaseValues = {
            ...
            {"contactswithfinance" , {"/contacts?fields=[metadata,workScheduleId,currentBillRate,currentCostRate,jobRoleId]","[]"} },
            {"contacts" , {"/contacts?fields=[metadata,workScheduleId,jobRoleId]","[]"} },
