...
SingleStoreConnectorMetaMain = type function (
...
GetODBCDataSource = (Database as text, ConnectionString as record, optional options as record) =>
    let
        //
        // Handle credentials
        ...
        // It's here to ensure compatibility with first implementation of SSL functionality
        // if options has "UseSSL" and it's not null, use that; otherwise default to null
        useSSL =
            if (options <> null and Record.HasFields(options, "UseSSL") and options[UseSSL] <> null) then
                options[UseSSL]
            else
                null,
        ...
        OdbcDatasource = Odbc.DataSource(ConnectionStringWithSSL, [
...
SingleStoreConnectorImplMain = (ServerAddr as text, Database as nullable text, optional options as record) =>
    let
        //
        ...
        OdbcDataSource = GetODBCDataSource(Database, ConnectionString, options)
    in
        OdbcDataSource;
    ...
    let
        ...
        BaseConnectionString =
                [
                    ...
                    Database = Database,
                    ApplicationIntent = "readonly",
                    APP = "SingleStore Power BI Direct Query Connector (with UseSSL parameter)",
                    no_ssps=1,
                    NO_CACHE=1
                ],
        ConnectionString = AddServerAndPort(BaseConnectionString, ServerAddr),
        OdbcDataSource = GetODBCDataSource(Database, ConnectionString, [UseSSL = UseSSL])
    in
        OdbcDataSource;
...
SingleStoreConnectorImplCustomSQL = (ServerAddr as text, Database as nullable text, optional Query as text) =>
    let
        ...
        ConnectionString = AddServerAndPort(BaseConnectionString, ServerAddr),
        DataSource = GetODBCDataSource(Database, ConnectionString),
        // Strip off any trailing semicolons — Value.NativeQuery will error if they’re present,
        // so we remove them to keep legacy queries ending in “;” working smoothly.
        QueryWithoutSemicolumns = Text.TrimEnd(Query, ";"),
        OdbcDataSource =  if QueryWithoutSemicolumns <> null then Value.NativeQuery(DataSource, QueryWithoutSemicolumns, null, [ EnableFolding = true ]) else DataSource
    in
        OdbcDataSource;

[DataSource.Kind="SingleStoreODBC", Publish="SingleStoreODBC.Publish"]
shared SingleStoreODBC.Contents = Value.ReplaceType(SingleStoreConnectorImplMain, SingleStoreConnectorMetaMain);
