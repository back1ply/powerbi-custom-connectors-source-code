...
DenodoForPowerBIImpl = (DSN as text, optional debug as nullable logical, optional options as record) =>
    let
        ...
        _Result_DSN = Validation.ProcessDSNParameter(DSN),
        ...
        QueryTimeoutOption = options[QueryTimeout]?,

        _QueryTimeout = if (QueryTimeoutOption <> null) then QueryTimeoutOption else null,

        QueryTimeoutIsNotNull = if (_QueryTimeout <> null) then true else false,

        ValidateQueryTimeout = if QueryTimeoutIsNotNull then if (Value.Is(_QueryTimeout, Number.Type)) then true else error Error.Record("Expression.Error", "The value entered for QueryTimeout is not of the correct type.", null) else false,

        _Result_DSN_Querytimeout = if ValidateQueryTimeout then Record.AddField(_Result_DSN, "ConnSettings", Text.Combine({"set+querytimeout+to+", Text.From(QueryTimeoutOption)})) else _Result_DSN,

        CompressionOption = options[Compression]?,

        _Compression = if (CompressionOption <> null) then CompressionOption else null,

        CompressionIsNotNull = if (_Compression <> null) then true else false,

        ValidateCompression = if CompressionIsNotNull then if (Value.Is(_Compression, Number.Type)) then true else error Error.Record("Expression.Error", "The value entered for Compression is not of the correct type.", null) else false,

        _Result_DSN_Compression = if ValidateCompression then Record.AddField(_Result_DSN_Querytimeout, "Compression", _Compression) else _Result_DSN_Querytimeout,

        UserAgentOption = options[UserAgent]?,

        _UserAgent = if (UserAgentOption <> null) then UserAgentOption else null,

        UserAgentIsNotNull = if (_UserAgent <> null) then true else false,

        ValidateUserAgent = if UserAgentIsNotNull then if (Value.Is(_UserAgent, Text.Type)) then true else error Error.Record("Expression.Error", "The value entered for UserAgent is not of the correct type.", null) else false,

        _Result_DSN_UserAgent = if ValidateUserAgent then Record.AddField(_Result_DSN_Compression, "UserAgent", _UserAgent) else _Result_DSN_Compression,

        Result_DSN = Record.AddField(_Result_DSN_UserAgent, "DecodeConnectionStringSecrets", 0),

        Credential = Extension.CurrentCredential(),

        AuthKind = Credential[AuthenticationKind],

        Result_DSN_Auth1 = if (AuthKind = "OAuth") then Record.AddField(Result_DSN, "UseOAuth2", 1) else Result_DSN,

        Result_DSN_Auth2 = if (AuthKind = "OAuth") then Record.AddField(Result_DSN_Auth1, "AccessToken", Credential[access_token]) else Result_DSN,
        
        _ConnectionString = Result_DSN_Auth2,
        ...
        SQLGetInfo = [
            ...
            SQL_STRING_FUNCTIONS = Flags({ SQL_FN_STR_ASCII, SQL_FN_STR_CHAR, SQL_FN_STR_CONCAT, SQL_FN_STR_INSERT, SQL_FN_STR_LCASE, SQL_FN_STR_LEFT, SQL_FN_STR_LENGTH, SQL_FN_STR_LOCATE, SQL_FN_STR_LOCATE_2, SQL_FN_STR_LTRIM, SQL_FN_STR_REPEAT, SQL_FN_STR_RIGHT, SQL_FN_STR_RTRIM, SQL_FN_STR_SPACE, SQL_FN_STR_SUBSTRING, SQL_FN_STR_UCASE, SQL_FN_STR_CHAR_LENGTH, SQL_FN_STR_CHARACTER_LENGTH, SQL_FN_STR_REPLACE }),
            
            SQL_NUMERIC_FUNCTIONS = Flags({ SQL_FN_NUM_ABS, SQL_FN_NUM_ACOS, SQL_FN_NUM_ASIN, SQL_FN_NUM_ATAN, SQL_FN_NUM_ATAN2, SQL_FN_NUM_CEILING, SQL_FN_NUM_COS, SQL_FN_NUM_COT, SQL_FN_NUM_DEGREES, SQL_FN_NUM_EXP, SQL_FN_NUM_FLOOR, SQL_FN_NUM_LOG, SQL_FN_NUM_MOD, SQL_FN_NUM_POWER, SQL_FN_NUM_RADIANS, SQL_FN_NUM_RAND, SQL_FN_NUM_ROUND, SQL_FN_NUM_SIGN, SQL_FN_NUM_SIN, SQL_FN_NUM_SQRT, SQL_FN_NUM_TAN, SQL_FN_NUM_PI }), 

            SQL_TIMEDATE_FUNCTIONS = Flags({ SQL_FN_TD_CURRENT_DATE, SQL_FN_TD_CURRENT_TIME, SQL_FN_TD_CURRENT_TIMESTAMP, SQL_FN_TD_CURDATE, SQL_FN_TD_CURTIME, SQL_FN_TD_DAYNAME, SQL_FN_TD_DAYOFMONTH, SQL_FN_TD_DAYOFWEEK, SQL_FN_TD_DAYOFYEAR, SQL_FN_TD_EXTRACT, SQL_FN_TD_HOUR, SQL_FN_TD_MINUTE, SQL_FN_TD_MONTH, SQL_FN_TD_MONTHNAME, SQL_FN_TD_NOW, SQL_FN_TD_QUARTER, SQL_FN_TD_SECOND, SQL_FN_TD_TIMESTAMPADD, SQL_FN_TD_TIMESTAMPDIFF, SQL_FN_TD_WEEK, SQL_FN_TD_YEAR }),
        ...
        OdbcDataSource = Odbc.DataSource(ConnectionString, [
            ...
            CredentialConnectionString =
                if (Credential[AuthenticationKind]? = "UsernamePassword") then
                    ...
                    [UseOauth2 = 1, AccessToken = Credential[access_token]]
                else
...
DenodoForPowerBI = [
    ...
    Authentication = [
        ...
        Aad = [
            ...
            Resource = "https://analysis.windows.net/powerbi/connector/DenodoForPowerBI"
        ]
    ],
