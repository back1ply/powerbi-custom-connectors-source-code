...
SupermetricsClient = let
    this.scheduleQuery = (queryParams as record) as record =>
        let
            ...
            _queryParamsWithSync0 = Record.AddField(queryParams, "sync_timeout", 0),
        ...
        in
            _result,
    this.getQueryStatus = (scheduleId as text, attempt as number) as text =>
        let
            _clusterUrl = this.getClusterUrl(),
            _url = Text.Combine({"https:/", _clusterUrl, Urls[Enterprise], "query/status"}, "/"),
            _queryParams = [
                #"schedule_id" = scheduleId,
                // we need it as a cache booster, since HttpClient caches GET-requests results
                #"check_attempt" = Text.From(attempt)
            ],
    ...
    this.getQueryResults = (scheduleId as text) as record =>
        ...
        in
            _result,
    this.getQueryResults2 = (queryParams as record, optional attempt as number) as record =>
        let
            _attemptNumber = if (attempt = null) then 0 else attempt,
            _scheduleResponse = if (_attemptNumber = 0) then this.scheduleQuery(queryParams) else
                [],
            _statusBuilder = (scheduleResponse as record) as text => this.getQueryStatus(queryParams[schedule_id], _attemptNumber),
        ...
        in
            if (_needWaitForResults = true) then
                Function.InvokeAfter(() => @this.getQueryResults2(queryParams, _attemptNumber + 1), #duration(0, 0, 0, 5))
...
Navigation = let
    ...
    this.dataTableWithProfileBuilder = (queryInfo as record, profileLabel as text) =>
        let
            result = Value.ReplaceType(
            (optional inputProfile as text, optional selectedDateRangeType as text) =>
                let
                    _queryDetails = this.getQueryDetails(queryInfo),
                    _userProfile =
        ...
        in
        result,
    this.dataTableWithAccountsAndDateFieldBuilder = (queryInfo as record, accounts as list) as function =>
        let
            result = Value.ReplaceType(
                (optional selectedConnectionAccounts as list, optional selectedDateRangeType as text) =>
                    let
                        _queryDetails = this.getQueryDetails(queryInfo),
                        _queryParams = _queryDetails[query_params],
                        _userAccounts =
    ...
    this.getLogins = () as list =>
        ...
        in
            result,
    this.getConnectionsTableWithAccountSelection = (queryInfo as record, datasourceConfig as record, withOptions as text) as function =>
        let
        ...
        in 
            _result
        ,
    this.getConnectionsTableWithInputFieldAsProfile = (queryInfo as record, profileLabel as text) as function =>
        let
            result = this.dataTableWithProfileBuilder(queryInfo, profileLabel)
        in
            result,

    this.getQueriesAndConnectionsBasedOnReportType = (queryInfo as record) as function =>
        let
            _queryDetails = this.getQueryDetails(queryInfo),
